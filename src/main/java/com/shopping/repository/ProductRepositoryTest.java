...
public class ProductRepositoryTest {
    ...
    @Test
    @DisplayName("@Query를 사용한 상품조회 테스트 01")
    ...
    @PersistenceContext // JPA 가 동작하는 가상의 공간(space), 이 이노테이션이 em에게 의미 있는 데이터를 주입 해 줍니다.
    EntityManager em ; //엔터티 관리자
    @Test
    @DisplayName("query Dsl Test01")
    public void queryDslTest01(){
        // 상품중에서 현재 판매중(SELL)인 상품을 가격 내림차순으로 조회합니다.
...
//        order by price desc ;
    ...
    public void queryDslTest02(){
        //현재 판매중(SELL)인 상품 중에서 , 단가가 200 초과이고, 세부 설명에 '어'가  포함되어있는 상품을 조회해 보세요.
        ...
        int price = 200 ;
        booleanBuilder.and(qbean.price.goe(price)) ;
